CREATE OR REPLACE FUNCTION Service$DropFunction(name TEXT)
  RETURNS text AS
$BODY$
DECLARE
    funcrow RECORD;
    ...
    paramtext text;
BEGIN
FOR funcrow IN SELECT proargtypes FROM pg_proc WHERE proname = LOWER(name) LOOP
    ...
    EXECUTE 'DROP FUNCTION ' || name || '(' || paramtext || ');';
